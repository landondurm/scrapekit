 ...
 */
export async function crawlLinks(page, baseUrl, maxDepth = 2, idleTimeout = 5000) {
  const origin = new URL(baseUrl).origin;
  ...
  while (queue.length > 0) {
    ...
    try {
      // Fast navigation condition, then settle to idle best-effort — a hard
      // networkidle wait burns the full 30s on sites that hold a connection open.
      ...
      await page.waitForLoadState('networkidle', { timeout: idleTimeout }).catch(() => {
        console.log(`[crawl] Network still active after ${idleTimeout}ms on ${normalized} — extracting links anyway`);
      });
